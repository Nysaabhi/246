...
function initializeBookingForm() {
  ...
  bookingForm.addEventListener('submit', function(e) {
    ...
    // Show confirmation
    showHappyConfirmation(customerName, bookingTotal.toFixed(2));
...
function showHappyConfirmation(customerName, bookingTotal) {
  const happyModal = document.getElementById('happy-confirmation-modal');
  const happyMessage = document.getElementById('happy-confirmation-message');
  
  if (happyModal && happyMessage) {
    // Set the confirmation message with customer details
    happyMessage.innerHTML = `
      <p>Thank you, <strong>${customerName}</strong>!</p>
      <p>Your booking of <strong>₹${bookingTotal}</strong> has been confirmed.</p>
      <p>We've sent the details to your registered contact.</p>
    `;
    
    // Show the modal with animations
    happyModal.classList.add('active');
    document.body.style.overflow = 'hidden';
    
    // Trigger the checkmark animation
    const happySiren = happyModal.querySelector('.happy-siren i');
    if (happySiren) {
      happySiren.style.animation = 'none';
      void happySiren.offsetWidth; // Trigger reflow
      happySiren.style.animation = 'checkmark 0.6s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
    }
    
    // Trigger the ripple effect
    const ripple = happyModal.querySelector('.happy-siren');
    if (ripple) {
      ripple.style.setProperty('--ripple-color', '#4CAF50');
    }
  }
}

function closeHappyConfirmation() {
  const happyModal = document.getElementById('happy-confirmation-modal');
  if (happyModal) {
    happyModal.classList.remove('active');
    document.body.style.overflow = '';
    
    // Reset animations
    const happySiren = happyModal.querySelector('.happy-siren i');
    if (happySiren) {
      happySiren.style.animation = '';
    }
  }
}
